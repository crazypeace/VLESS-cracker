...
//   Original flow: connect → send ClientHello → read response → send probe → read response
//   Modified flow: connect → send [probe + ClientHello] → read response
//
// Probe bytes and captured ClientHello are concatenated and sent as one shot.
// The server sees: CCS record + ClientHello record in the same TCP segment.
//
// Usage: same as original vless-cracker-v1:
//   sudo ./probe-18-mimo -i docker0 -f "tcp port 8443 and host X and host Y" -P probes-18.txt -l info
...
static void usage(const char *argv0) {
    fprintf(stderr,
            ...
            "\n"
            "*** PROBE-FIRST (probe-18-mimo): probe + ClientHello sent as one shot ***\n"
            "*** Original: connect → send ClientHello → read response               ***\n"
            "*** Modified: connect → send [probe + ClientHello] → read response     ***\n"
            "\n"
            "Examples:\n"
            "  sudo %s -i eth0\n"
            "  sudo %s -i docker0 -f \"tcp port 8443 and host 172.17.0.1 and host 172.17.0.2\" -P probes-18.txt -l info\n",
            argv0, argv0, argv0);
}
...
static int replay_client_hello_to_server_once(App *app, const FlowKey *server_key,
    ...
    /* === PROBE-FIRST MODE: send probe + ClientHello as one shot === */
    if (app->probe_count > 0) {
        /* Calculate total probe bytes */
        size_t total_probe_len = 0;
        for (size_t pi = 0; pi < app->probe_count; pi++) {
            total_probe_len += app->probes[pi].len;
        }

        /* Build combined buffer: probe bytes + ClientHello */
        size_t combined_len = total_probe_len + client_hello_len;
        uint8_t *combined = malloc(combined_len);
        if (!combined) {
            log_infof("TLS PROBE-FIRST server=%s:%u malloc failed for combined buffer\n",
                      server_addr, server_key->sport);
            close(fd);
            if (status_out) {
                *status_out = REPLAY_STATUS_SEND_FAILED;
        ...
        size_t off = 0;
        for (size_t pi = 0; pi < app->probe_count; pi++) {
            memcpy(combined + off, app->probes[pi].bytes, app->probes[pi].len);
            off += app->probes[pi].len;
        }
        memcpy(combined + off, client_hello_record, client_hello_len);

        log_infof("TLS PROBE-FIRST server=%s:%u sending probe(%zu) + ClientHello(%zu) = %zu bytes combined\n",
                  server_addr, server_key->sport,
                  total_probe_len, client_hello_len, combined_len);

        if (send_all_with_timeout(fd, combined, combined_len,
                                  REPLAY_TIMEOUT_MS) == -1) {
            log_infof("TLS PROBE-FIRST server=%s:%u combined send_failed error=%s\n",
                      server_addr, server_key->sport, strerror(errno));
            free(combined);
            close(fd);
            if (status_out) {
                *status_out = REPLAY_STATUS_SEND_FAILED;
    ...
    /* === END PROBE-FIRST MODE === */

    if (app->probe_count == 0 &&
        send_all_with_timeout(fd, client_hello_record, client_hello_len,
                              REPLAY_TIMEOUT_MS) == -1) {
